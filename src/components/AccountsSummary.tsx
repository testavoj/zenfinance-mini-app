    ...
    <div className="bg-white dark:bg-white/5 border border-zinc-200 dark:border-white/10 rounded-[3rem] p-8 space-y-6 shadow-sm dark:shadow-none">
      ...
      {transactions.length === 0 ? (
        <div className="py-6 text-center space-y-3">
          <p className="text-sm font-bold text-zinc-900 dark:text-white">{t('noTxYet')}</p>
        ...
        <div className="text-center py-2">
          <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest">{t('netBalance')}</p>
          ...
          <p className="text-[10px] text-zinc-500 mt-1">{t('entriesLogged', { n: transactions.length })}</p>
        </div>
      )}
